...
ROOT = os.getcwd().split('/scripts')[0].strip()
RESULTS_ROOT = f"{ROOT}/icpp_training_results"
DATA_ROOT = f"{ROOT}/protein_dataset"
...
def load_data():
    ...
    for root, dirs, files in sorted(os.walk(RESULTS_ROOT)):
    ...
    print(f"Length of data_files: {len(data_files)}")
...
def make_figure2():
    df = pd.read_csv(f'{RESULTS_ROOT}/1gpu/no_early_termination/1gpu_1e15_training_data.csv')
...
def plot_times():
    time_df = pd.read_csv(f'{RESULTS_ROOT}/time_to_run.csv')
...
def make_figure9():
    ...
    time_df.to_csv(f'{RESULTS_ROOT}/time_to_run.csv', index=False)
    plot_times()
    return 
